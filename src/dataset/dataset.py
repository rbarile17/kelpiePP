...
from .. import FB15K, FB15K_237, WN18, WN18RR, YAGO3_10
...
class Dataset:
    def __init__(self, dataset: str, test_sample=False):
        self.name = dataset
        if dataset == "YAGO4-20":
            ...
            self.dataset = get_dataset(
                ...
                testing=DATA_PATH / dataset / f"{test_name}_named.txt",
                validation=DATA_PATH / dataset / "valid_named.txt",
            )
        elif dataset in [FB15K, WN18, YAGO3_10]:
            test_name = "sample_test" if test_sample else "test"
            self.dataset = get_dataset(
                training= DATA_PATH / dataset / "train.txt",
                testing=DATA_PATH / dataset / f"{test_name}.txt",
